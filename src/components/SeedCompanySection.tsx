...
//     img: "https://images.unsplash.com/photo-1589923188900-85dae523342b?w=1400&auto=format&fit=crop&q=80",
//     imgMob: "https://images.unsplash.com/photo-1589923188900-85dae523342b?w=800&q=70&auto=format&fit=crop",
//     mTitle1: "Cleaned &",
...
//   color: var(--sc-card-accent); margin-bottom: clamp(.5rem,1vw,1rem);
//   display: flex; align-items: center; gap: 12px;
// }
...
//   font-weight: 300; line-height: .9; color: var(--sc-card-text); letter-spacing: -.01em;
// }
// .sc-card-title em {
...
//   line-height: 1.75; color: rgba(255,255,255,.75); font-weight: 300;
// }
...
//   .sc-m-tag { display: inline-block; font-size: 9px; letter-spacing: .15em; text-transform: uppercase; color: var(--sc-card-accent); border: 1px solid var(--sc-card-accent); padding: 2px 8px; border-radius: 2px; margin-bottom: .6rem; }
//   .sc-m-title { font-family: 'Cormorant Garamond', serif; font-size: clamp(2.4rem,11vw,4rem); font-weight: 300; line-height: .9; color: var(--sc-card-text); }
//   .sc-m-title em { font-style: italic; color: rgba(255,255,255,.55); font-size: .7em; display: block; }
//   .sc-m-desc { margin-top: .5rem; font-size: 12px; line-height: 1.65; color: rgba(255,255,255,.75); }
// }
// `;
...
const CARDS_DATA = [
  {
    ...
    imgMob: "https://i2.wp.com/www.africaspeaks4africa.net/wp-content/uploads/2018/11/seeds.jpg?w=800&ssl=1",
    mTitle1: "Choosing the",
    mTitle2: "Finest Seeds",
    mDesc: "Hand-picked varieties screened for yield, resistance, and regional adaptability.",
  },
  {
    ...
    imgMob: "https://www.sgs.com/-/media/sgscorp/images/temporary/scientist-analysing-wheat-seeds.cdn.en-PT.1.jpg",
    mTitle1: "Tested in our",
    mTitle2: "State-of-Art Lab",
    mDesc: "ISO-certified germination, purity, moisture and pathogen screening on every batch.",
  },
  {
    ...
    imgMob: "https://brandonbioscience.com/wp-content/uploads/2024/05/shutterstock_2414814355-1536x1024.webp",
    mTitle1: "Proven in the",
    mTitle2: "Real Field",
    mDesc: "Trialed across 12+ locations in varying soils and climates before approval.",
  },
  {
    ...
    imgMob: "https://gdagroindustries.com/wp-content/uploads/2025/07/seed-and-paddy-processing-plant-gd-agro-industries-india.webp",
    mTitle1: "Cleaned &",
    mTitle2: "Sealed Fresh",
    mDesc: "Graded, treated and moisture-sealed with full batch traceability in 48 hours.",
  },
  {
    ...
    imgMob: "https://tcifreight.in/wp-content/uploads/2025/06/seeds-fams.jpg",
    mTitle1: "Straight to",
    mTitle2: "Your Farm",
    mDesc: "Dispatched within 24 hours in temperature-controlled packs, across India.",
  },
...
export default function SeedCompanySection() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      /* ══ MOBILE STACKING ENGINE ══ */
      if (isMobile()) {
        const anchors = document.querySelectorAll<HTMLElement>(".sc-m-stack-anchor");
        const cards = document.querySelectorAll<HTMLElement>(".sc-m-stack-card");
        ...
        cards.forEach((card, i) => {
          ...
          ScrollTrigger.create({
            trigger: anchor,
            start: "top bottom",  // when anchor enters from bottom
            end: "top top",       // when anchor hits the very top
            scrub: true,
            onUpdate(self: any) {
              const p = self.progress;
              // scale from 0.84 → 1, rotate from -4° → 0, opacity 0.35 → 1
              const scale = 0.84 + 0.16 * p;
              const rotate = -4 + 4 * p;
              const opacity = 0.35 + 0.65 * p;
              gsap.set(card, { scale, rotation: rotate, opacity, transformOrigin: "center top" });
            },
          });
        });

        return; // skip desktop engine
      }
  ...
  return (
    <>
      ...
      <div id="sc-mobile">
        ...
        <div className="sc-m-stack-section">
          {CARDS_DATA.map((c, i) => (
            <div
              key={c.id}
              className="sc-m-stack-anchor"
              style={{ paddingTop: `${i * 20}px` }}
            >
              <div className="sc-m-stack-card" data-index={i}>
                <div className="sc-m-stack-imgwrap">
                  <img src={c.imgMob} alt={c.tag} loading="lazy" />
                  <div className="sc-m-stack-overlay" />
                  <div className="sc-m-stack-num">{`0${i + 1}`}</div>
                </div>
                <div className="sc-m-stack-body">
                  <span className="sc-m-stack-tag">{c.tag}</span>
                  <div className="sc-m-stack-title">
                    <em>{c.mTitle1}</em>
                    {c.mTitle2}
                  </div>
                  <p className="sc-m-stack-desc">{c.mDesc}</p>
                  <div className="sc-m-stack-stat">
                    <strong>{c.stat}</strong>
                    <span>{c.statLabel}</span>
                  </div>
                  <div className="sc-m-stack-progress">
                    <div className="sc-m-stack-progress-fill" style={{ width: `${((i + 1) / CARDS_DATA.length) * 100}%` }} />
                  </div>
                  <div className="sc-m-stack-counter">{i + 1} / {CARDS_DATA.length}</div>
                </div>
              </div>
            </div>
          ))}
...
@media (max-width: 768px) {
  ...
  .sc-m-eyebrow {
    ...
    color: var(--sc-accent); margin-bottom: 1.1rem;
  }
  .sc-m-hero h1 {
    font-family: 'Cormorant Garamond', serif;
  ...
  /* Each anchor gives the card ~90vh of scroll travel */
  .sc-m-stack-anchor {
    position: sticky;
    top: 0;
    width: 100%;
  ...
  /* The card itself: full width, rounded box */
  .sc-m-stack-card {
    width: 100%;
    border-radius: 20px;
    overflow: hidden;
  ...
  .sc-m-stack-imgwrap img {
    width: 100%; height: 100%; object-fit: cover;
    display: block; transition: transform 0.6s ease;
  }
  .sc-m-stack-overlay {
    position: absolute; inset: 0;
    background: linear-gradient(135deg, rgba(0,0,0,.55) 0%, rgba(0,0,0,.08) 60%, transparent 100%);
  }
  .sc-m-stack-num {
  ...
  .sc-m-stack-body {
    ...
    background: linear-gradient(to bottom, #141a0e 0%, #0d1208 100%);
    flex: 1;
  }
  .sc-m-stack-tag {
  ...
  .sc-m-stack-title em {
    ...
    font-size: .65em; display: block;
    margin-bottom: .15em; letter-spacing: .02em;
  }
  .sc-m-stack-desc {
  ...
  .sc-m-stack-progress {
    ...
    border-radius: 1px; overflow: hidden; margin-bottom: .5rem;
  }
  .sc-m-stack-progress-fill {
    height: 100%;
    background: linear-gradient(to right, var(--sc-card-accent), rgba(140,198,63,.4));
    border-radius: 1px;
  }
  .sc-m-stack-counter {
